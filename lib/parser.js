...
class Parser {
  ...
  parseItemRss(xmlItem, itemFields) {
    ...
    if (xmlItem.image && xmlItem.image[0] && xmlItem.image[0].url) {
      item.image = {};
      let image = xmlItem.image[0];
      if (image.link) item.image.link = image.link[0];
      if (image.url) item.image.url = image.url[0];
      if (image.title) item.image.title = image.title[0];
      if (image.width) item.image.width = image.width[0];
      if (image.height) item.image.height = image.height[0];
    }
